...
'''
In a /bin/sh script I mean not a bash or zsh script.:
1. I need to find out which distro of linux it is.
2. If it's fedora linux then I want to do some task like below.

Task list..
1. Find out zsh is installed or not. If not installed then install and make it default shell.
2. Check out oh-my-zsh is installed or not. if not then install it and ask to restart the terminal and re-run the script to continue.
3. install "cascadia-code-nf-fonts" with dnf.
4. dowload and install below fonts.
  1. https://github.com/ryanoasis/nerd-fonts/releases/download/v3.2.1/CascadiaCode.zip
  2. https://github.com/ryanoasis/nerd-fonts/releases/download/v3.2.1/FiraCode.zip
  3. https://github.com/ryanoasis/nerd-fonts/releases/download/v3.2.1/Meslo.zip
5. Execute the below commands to install the powerlevel10k theme and plugins, before install check if it's installed or not.
  git clone --depth=1 https://github.com/romkatv/powerlevel10k.git ${ZSH_CUSTOM:-$HOME/.oh-my-zsh/custom}/themes/powerlevel10k
  git clone --depth=1 https://github.com/zsh-users/zsh-completions ${ZSH_CUSTOM:-$HOME/.oh-my-zsh/custom}/plugins/zsh-completions
  git clone --depth=1 https://github.com/zsh-users/zsh-autosuggestions ${ZSH_CUSTOM:-$HOME/.oh-my-zsh/custom}/plugins/zsh-autosuggestions
  git clone --depth=1 https://github.com/zsh-users/zsh-syntax-highlighting.git ${ZSH_CUSTOM:-$HOME/.oh-my-zsh/custom}/plugins/zsh-syntax-highlighting
6. Edit the ~/.zshrc file and change the below lines.
